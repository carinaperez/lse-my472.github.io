...
r <- GET(base_url, query = list(q = "inequality", "api-key" = key_get(key_name)))
r
```
...
Question: Between these two dates, how many articles in the NYT mentioning "inequality"? 1463.
...
Let's gather data about mentions of "Obama" over the period January 1 2007 through December 31st 2012 (again, this will take a while to run):
...
```{r}
exemplary_archive_url <- sprintf("https://api.nytimes.com/svc/archive/v1/%g/%g.json?api-key=%s", 1855, 7, key_get(key_name))
